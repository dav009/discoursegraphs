...
def __walk_chain(rel_dict, from_id):
    ...
    Returns
    -------
    chains_starting_with_from_id : list of list of str
        each list constains a list of strings (i.e. a list of node IDs,
        which represent a chain of pointing relations)
    """
    chains_starting_with_from_id = []

    for to_id in rel_dict[from_id]:
        if to_id in rel_dict:
            for tail in __walk_chain(rel_dict, to_id):
                chains_starting_with_from_id.append([from_id] + tail)
        else:
            chains_starting_with_from_id.append([from_id, to_id])
    return chains_starting_with_from_id
...
def get_pointing_chains(docgraph):
    ...
    return all_chains  # TODO: return unique_chains after debugging!
